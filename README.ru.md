...
Метеостанция на ESP32-S3 с большим e-ink дисплеем, питанием от 18650 и 3D-печатным корпусом для настенного использования.

При обновлении раз в 2 часа типичный расход составляет около `5-6 мА·ч/день`, что даёт примерно `~1 год работы от аккумулятора 2000 мА·ч` или `~1.5 года от 3000 мА·ч`.

## Ключевые фишки

- Большая автономность за счёт deep sleep между обновлениями.
- Экран 800x480 с текущей погодой, 48-часовым графиком и 6-дневным прогнозом.
- Быстрое обновление экрана, достаточное для комфортного повседневного использования большого 3.97" e-ink дисплея.
- Встроенная веб-админка: Wi-Fi, провайдер погоды, локация и интервал обновления меняются без перепрошивки.
- Два источника погоды: бесплатный Open-Meteo без API-ключа и OpenWeatherMap One Call 3.0.
- Симулятор в [`sim/`](sim/) для отладки интерфейса без постоянной прошивки железа.
- Готовые STL-файлы корпуса для полноценного настенного устройства.

![Inkcast device](assets/inkcast.png)

Устройство показывает текущую погоду, 48-часовой график температуры и осадков, 6-дневный прогноз, заряд батареи, уровень WiFi, восход и закат, а также счётчик солнечных дней на e-ink экране, который остаётся читаемым без подсветки.

## Данные на экране

- Блок текущей погоды с иконкой, ощущаемой температурой, минимумом и максимумом, вероятностью осадков, восходом, закатом и счётчиком солнечных дней.
- 48-часовой график температуры и осадков.
- 6-дневный прогноз с иконками и дневными минимумами и максимумами.
- Автоматическое определение часового пояса по координатам через погодное API.
- Настраиваемый интервал обновления от 1 до 1440 минут.
- Runtime-настройки в NVS, которые переживают deep sleep и перезагрузки.
- Режим отладки с логами и встроенной веб-админкой без изменения прошивки.
- Резервная точка доступа для первичной настройки или восстановления после сломанных WiFi-настроек.

## Источники погоды

- **Open-Meteo** — бесплатно, без API ключа, доступен из России. Можно выбирать модель прогноза.
- **OpenWeatherMap One Call 3.0** — требует API ключ и работает как альтернативный источник данных.

По умолчанию устройство обновляется раз в 2 часа, хранит runtime-настройки в NVS и всё остальное время проводит в deep sleep.
